...
class GPT(nn.Module):
    ...
    def setup_optimizer(self, unembedding_lr=0.004, embedding_lr=0.2, matrix_lr=0.02,
        ...
        param_groups = [
            ...
            dict(kind='adamw', params=x0_params, lr=scalar_lr, betas=(0.99, 0.999), eps=1e-10, weight_decay=0.0),
        ]
    ...
    def forward(self, idx, targets=None, reduction='mean'):
        ...
        if targets is not None:
            ...
            # Logit lens consistency: penalize divergence between penultimate and final
            if CONVERGENCE_COEFF > 0 and penultimate is not None:
                with torch.no_grad():
                    final_probs = F.softmax(logits, dim=-1)
                inter_logits = self.lm_head(norm(penultimate)).float()
                inter_logits = softcap * torch.tanh(inter_logits / softcap)
                inter_log_probs = F.log_softmax(inter_logits, dim=-1)
                conv_loss = F.kl_div(inter_log_probs, final_probs, reduction='batchmean')
                loss = loss + CONVERGENCE_COEFF * conv_loss
            return loss
        return logits
...
WINDOW_PATTERN = "SL"   # sliding window pattern: alternating short/long

# Optimization
TOTAL_BATCH_SIZE = 2**14 # ~16K tokens per optimizer step (reduced for 24GB MPS)
...
CONVERGENCE_COEFF = 0.01 # logit lens consistency loss coefficient

# Model size
DEPTH = 8               # number of transformer layers
DEVICE_BATCH_SIZE = 8   # per-device batch size (reduced for 24GB MPS)
...
# Save checkpoint for interpretability analysis
CHECKPOINT_DIR = os.path.join(os.path.expanduser("~"), ".cache", "autoresearch", "checkpoints")
os.makedirs(CHECKPOINT_DIR, exist_ok=True)
checkpoint_path = os.path.join(CHECKPOINT_DIR, "latest.pt")
# Save state_dict + train.py source so interpret.py can reconstruct any architecture
